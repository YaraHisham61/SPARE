# GEMINI.md — Instructions for Gemini Code in Click2Act

This file governs how Gemini Code must behave in this research project.
...
Every file Gemini **creates** must begin with a header block:
...
When Gemini edits an **existing** line (spelling, logic, wording, structure), mark the change inline so the diff is human-readable inside the file itself:
...
All Gemini-authored or Gemini-assisted markdown files (notes, summaries, literature reviews, experiment logs, analyses) **must** live under:
...
Example: `docs/gemini/2026-03-30_omniparser-summary.md`
...
The folder `docs/gemini/` is for **AI-assisted** research notes.
Do not mix them.
...
The canonical folder structure for this project is defined in `README.md`. Gemini must:
...
| Gemini-assisted research docs | `docs/gemini/`                 |
...
### After every edit, Gemini MUST append a "Word Suggestions" block
...
The user decides whether to accept any suggestion. Gemini never applies suggestions automatically.
...
- Gemini must propose a commit message whenever it makes a meaningful change, but never commit without explicit user approval.
...
- **No speculation without flagging**: if Gemini is uncertain, prefix the statement with `> [Uncertain]`.

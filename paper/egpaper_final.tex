...
%%%%%%%%% ABSTRACT
% REFINED [old]: "yet no systematic study" + specific dataset/mIoU numbers → [new]: softer "few studies" phrasing and qualitative results (numbers moved to body)
\begin{abstract}
  
  Promptable image segmentation has rapidly expanded to accept natural-language concept prompts alongside spatial cues, though few studies have measured how the resulting models cope with noise on both input channels.  We introduce \textbf{SPARE}, an output-stability framework that probes SAM~3 under three corruption families---Gaussian image noise, spatial flips, and character-level prompt substitutions---across the SA-1B and SACo-Gold benchmarks, comparing each degraded prediction to the model's own clean-input baseline rather than to human annotations. Under strong Gaussian noise the concept-prompt pipeline remains largely stable, while even a single-character prompt substitution sharply degrades it and a few more substitutions collapse output stability almost entirely; spatial flips, by contrast, produce mostly geometric misregistration rather than semantic failure.  Linguistic input, not visual quality, drives the dominant operational risk for concept-prompted segmentation in SAM~3.
...
% REFINED [old]: 9-line caption with per-axis details → [new]: 3-line caption (headline only)
\caption{\textbf{SAM~3 robustness landscape.}
Axes: Gaussian $\sigma\!\in\!\{10,25,50\}$, flips (H, V, H+V), text
substitutions $\ell\!\in\!\{1,2,3\}$; radius is mIoU vs.\ the clean-input
prediction (flip-back for flips).  SACo-Gold absorbs visual noise but
collapses under text noise; SA-1B uses a fixed prompt and is not
evaluated on text axes.}
...
The second pipeline is invoking SAM~3 with the concept-level text prompt stored in each SACo-Gold record drawn from the SA-1B captioner subset of SACo-Gold~\cite{meta2025saco} (13,258 image concept
...
We test three modes: horizontal~(\textbf{h}), vertical~(\textbf{v}), and combined~(\textbf{both}). All prompts are held fixed. This degradation is applied to \emph{both} pipelines.
  ...
  We measure output consistency using \textbf{flip-back mIoU}: the predicted
  mask $\hat{\mathbf{M}}_\phi$ is mapped back to the original coordinate frame
  via the inverse transformation $T_\phi^{-1}$ before comparison against the
  clean baseline $\hat{\mathbf{M}}_0$:
  %
  \begin{equation}
  ...
  \end{equation}
  %
  A high $\mathrm{mIoU}_{\mathrm{fb}}$ means the model correctly segmented
  the object in the flipped frame (equivariant behaviour); a low value
  indicates semantic failure that persists even after coordinate alignment.
%--------------------------------------------------------------------
...
To compare fragility across degradation types and prompt modalities on a common scale, we normalise the stability drop by the noise magnitude:
  ...
  %
  where $\ell$ is the noise level (number of character substitutions for linguistic noise; $\sigma$ for Gaussian noise). $S_\ell \leq 0$ always; a larger magnitude indicates greater fragility per unit of noise. Spatial flips are excluded from SI as their modes (h, v, both) are
  categorical and carry no meaningful numeric magnitude; flip results are
  characterised through flip-back mIoU (\S\ref{sec:flips}).
%-----------------------------------------------
...
file, one file per degradation level. This design allows each condition to be reproduced independently. mIoU and SI values are computed offline
...
Table~\ref{tab:flips} reports direct mIoU and flip-back mIoU for each flip mode and pipeline. Direct mIoU
...
target object in the flipped frame. Horizontal flips are consistently easier to recover than vertical or combined flips in both pipelines, likely
...
\begin{tabular}{llcc}
\hline
Pipeline & Mode & Direct & Flip-back \\
\hline
% REFINED [old]: \multirow{}{SA-1B} → [new]: \multirow{3}{*}{SA-1B} (correct syntax: rows, width, content)
\multirow{3}{*}{SA-1B}
  & Horizontal & 0.094 & 0.705 \\
  & Vertical   & 0.055 & 0.439 \\
  & Both       & 0.031 & 0.431 \\
\hline
% REFINED [old]: \multirow{SACo-Gold} → [new]: \multirow{3}{*}{SACo-Gold}
\multirow{3}{*}{SACo-Gold}
  & Horizontal & 0.204 & 0.892 \\
  & Vertical   & 0.134 & 0.692 \\
  & Both       & 0.090 & 0.685 \\
\hline
\end{tabular}
\vspace{3pt}
\caption{Spatial flip results.  Direct mIoU compares flipped predictions against the clean baseline without alignment. Flip-back mIoU applies the inverse transform before comparison.}
\label{tab:flips}
\end{table}
...
\section{Conclusion}
\label{sec:conc}
SPARE evaluates SAM 3 under systematic input corruption across two prompt modalities visual and linguistic. We adopt an output-stability protocol that measures prediction consistency rather than accuracy against fixed annotations, and benchmark the model under Gaussian noise, spatial flips, and typographic prompt corruptions across multiple severity levels.


Our results show that linguistic degradation poses the greater operational risk: a single character substitution collapses output consistency far below anything observed under visual noise. We also find that concept prompts improve visual robustness compared to generic prompting.


Future work should evaluate simultaneous visual and linguistic degradation, and test against realistic noise sources such as OCR errors to better reflect real-world deployment conditions.
%--------------------------------------------------------------------
{\small
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.82\linewidth]{figs/fig1_degradation_curves.png}
  \caption{mIoU vs.\ Gaussian noise level $\sigma$ for the SA-1B (generic
           prompt) and SACo-Gold (concept prompt) pipelines.  Both curves
           anchor at mIoU,=,1 for the clean baseline and decrease
           monotonically; SACo-Gold consistently retains higher stability,
           reflecting the added semantic context of concept prompts.}
  \label{fig:supp-degrad-curves}
\end{figure}


\begin{figure}[h]
  \centering
  \includegraphics[width=0.82\linewidth]{figs/fig4_prompt_length.png}
  \caption{Per-prompt-length mIoU under text degradation.  Single-word
           concepts collapse to near-zero stability at $\ell!=!1$
           (one character substituted), while three-or-more-word phrases
           retain substantial stability at the same noise level.  The gap
           is explained by the effective corruption rate
           $\eta = \ell/\bar{c}$: a single substitution corrupts 17% of
           characters in a 6-character word but only 6% of a 17-character
           phrase.}
  \label{fig:supp-prompt-length}
\end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[width=0.95\linewidth]{figs/fig5_cross_modal.png}
  \caption{Cross-modal comparison of output stability under visual
           (left) and linguistic (right) degradation in the SACo-Gold
           pipeline.  The y-axis is shared to enable direct comparison:
           visual mIoU degrades slowly (0.923 at $\sigma!=!10$), while
           linguistic mIoU collapses sharply (0.323 at $\ell!=!1$),
           illustrating the asymmetric fragility of the two input
           modalities.}
  \label{fig:supp-cross-modal}
\end{figure}

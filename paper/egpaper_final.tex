...
\label{sec:results}
%--------------------------------------------------------------------
\subsection{Linguistic Fragility by Prompt Length}                      \label{sec:results-text-length}                                        Table~\ref{tab:prompt-length-si} reports mean IoU and Sensitivity Index foreach word-count bucket across noise levels $\ell \in \{1,2,3\}$.
...
\begin{tabular}{lccc}
\hline 
Bucket & SI($\ell$=1) & SI($\ell$=2) & SI($\ell$=3) \\ 
\hline 
\texttt{1-word}   & -0.9733  & -0.4956  & -0.3327 \\ 
\texttt{2-word}   &-0.7923  & -0.4736  & -0.3276 \\ 
\texttt{3+-word}  &-0.5168  & -0.3733 &  -0.2884 \\      
\hline 
\end{tabular}
\vspace{3pt}
\caption{Sensitivity Index per prompt-length bucket under linguistic
degradation (SACo-Gold pipeline).  A larger magnitude indicates greater fragility per unit of noise. Effective corruption rates
at $\ell$=1 are 17\%, 11\%, and 6\% for 1-, 2-, and 3+-word buckets respectively.}

...
The release of the Segment Anything Model (SAM) \cite{kirillov2023sam} marked a paradigm shift in class-agnostic, zero-shot instance segmentation through point, box, and coarse mask prompting. This architecture was subsequently expanded to the video domain by SAM 2 \cite{ravi2024sam2}, introducing memory modules for temporal mask propagation. Most recently, SAM 3 \cite{peng2025sam3} bridged the gap between localization and high-level semantic interpretation by integrating explicit concept-level natural language prompting alongside traditional spatial indicators, utilizing foundational benchmarks like SACo-Gold \cite{meta2025saco}. While these models exhibit remarkable structural flexibility, their expansive capability surface introduces diverse vectors for behavioral failure under noisy inputs.

\subsection{Robustness and Corruption Benchmarks}
Evaluating neural networks against out-of-distribution (OOD) data and visual degradations is foundational to assessing real-world reliability. Pioneered by ImageNet-C \cite{hendrycks2019benchmarking}, the standard protocol heavily relies on applying synthetic pixel-level distortions (e.g., Gaussian noise, blurs, and environmental artifacts) to gauge model degradation. In web-scale multimodal spaces, frameworks like LAION-C \cite{li2025laionc} have adapted these principles to evaluate generalized visual models.

Closer to our domain, initial empirical audits of the original SAM architecture \cite{wang2023empirical} highlighted its vulnerabilities to extreme low-quality inputs, inspiring architectural modifications like RobustSAM \cite{chen2024robustsam} to enhance performance on degraded images. Furthermore, the BREPS framework \cite{moskalenko2026breps} formalized bounds on how spatial prompt inaccuracies (such as bounding-box jitter) degrade segmentation outputs. However, these benchmarks heavily focus on absolute accuracy drops against static human annotations under spatial or purely visual noise, leaving the self-consistency of newer concept-driven models unexamined.

\subsection{Prompt and Linguistic Sensitivity in VLMs}
As vision models grow increasingly reliant on textual guidance, they inherit the well-documented vulnerabilities of Vision-Language Models (VLMs) regarding prompt styling and linguistic perturbation. Recent systematic studies emphasize that multimodal architectures exhibit distinct behavioral fragility depending on prompt framing. For example, the SCAM benchmark \cite{materzynska2025scam} demonstrated how severe typographic noise and real-world misspellings degrade the reasoning paths of multimodal foundation networks. Concurrently, PARC \cite{schmalfuss2025parc} proposed a formal framework to expose systemic geometric asymmetries within VLMs when matching text expressions to visual scenes.


\subsection{Our Distinct Contribution (SPARE)}
While existing literature evaluates either visual degradation in isolation (\cite{chen2024robustsam, li2025laionc}), spatial prompt jitter (\cite{moskalenko2026breps}), or typographic foundation errors (\cite{materzynska2025scam}), SPARE bridges these domains. Rather than measuring absolute performance drops against static human annotations, we pioneer a dual-pipeline, output-stability paradigm to directly contrast visual versus concept-driven linguistic fragility. This allows us to quantify exactly where multimodal grounding introduces operational risks compared to generic segmentation modes.

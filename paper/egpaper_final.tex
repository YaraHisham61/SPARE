...
%%%%%%%%% TITLE
\title{SPARE: Stress-testing Prompt Robustness for SAM 3}
...
\paragraph{}
  We test three modes: horizontal~(\textbf{h}), vertical~(\textbf{v}), and
  combined~(\textbf{both}). All prompts are held fixed. This degradation is
  applied to \emph{both} pipelines.
...
\paragraph{}
Gaussian noise sampling and character substitutions involve stochastic operations. All experiments are run with a single random seed.

...
# Date   : 2026-05-12
# Prompt : Create mock evaluation generator with realistic degradation results (cleaned)
...
def generate_comprehensive_report(results_df, output_dir):
    ...
    report_lines.append("")
    report_lines.append("Visual degradation applies Gaussian noise (AWGN) to images at various sigma levels.")
    report_lines.append("")
    ...
    report_lines.append("Average mIoU drop by noise level (sigma):")
    for sigma, drop in vis_impact.items():
        if sigma == 0:
            report_lines.append(f"  - Noise sigma={sigma} (baseline): {drop:.4f} drop")
        else:
            report_lines.append(f"  - Noise sigma={sigma}: {drop:.4f} drop")
    report_lines.append("")
    ...
    report_lines.append("Most robust configuration:")
    ...
    report_lines.append("Least robust configuration:")
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"[OK] MOCK EVALUATION COMPLETE")
    print(f"{'='*70}")
